...
import org.catools.athena.rest.feign.configs.PipelineConfigs;
...
public class PipelineHelper {
    ...
    private static PipelineDto buildPipeline() {
        if (PipelineUtils.getProjectClient().getProject(PROJECT_CODE) == null) {
            PipelineUtils.getProjectClient().saveProject(
                    new ProjectDto()
        ...
        if (PipelineUtils.getEnvironmentClient().getEnvironment(ENVIRONMENT_CODE) == null) {
            PipelineUtils.getEnvironmentClient().saveEnvironment(
                    new EnvironmentDto()
    ...
    private static void addUserIfNotExists() {
    ...
    private static void addStatusIfNotExists(String status) {
